...
class Dashboard {
	...
	protected function __construct() {
		$this->menu_info = Utils::white_label()['admin_menu_page'] ?? array();
		$this->setup_hooks();
	}
	...
	public function add_admin_menu(): void {
		...
		add_menu_page(
			...
			$this->get_menu_icon(), // $this->menu_info['icon_url'] ?? ''.
	...
	public function is_menu_page(): bool {
		...
		$admin_scripts_bases = array( 'toplevel_page_' . BLANK_PLUGIN_NAME );
	...
	public function register_admin_assets(): void {
		...
		// Register styles.
		wp_register_style( 'blank-plugin-admin', BLANK_PLUGIN_BUILD_PATH_URL . "/css/admin{$suffix}.css", array( 'wp-components' ), filemtime( BLANK_PLUGIN_BUILD_PATH . "/css/admin{$suffix}.css" ), 'all' );

		// Enqueue Styles.
		wp_enqueue_style( 'blank-plugin-admin' );

		$asset_config_file = sprintf( '%s/js/admin.asset.php', BLANK_PLUGIN_BUILD_PATH );
		...
		wp_register_script(
			'blank-plugin-admin',
			BLANK_PLUGIN_BUILD_PATH_URL . '/js/admin.js',
			array_unique( array_merge( $js_dependencies, array() ) ),
			$version,
			true
		);
