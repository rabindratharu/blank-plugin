...
class Rest_Endpoint {
	...
	private const NAMESPACE = 'blank-plugin';

	private const DEFAULT_POSTS_PER_PAGE = 9;
	private const DEFAULT_PAGE           = 1;
	private const POST_TYPE              = 'review';
	 ...
	 * e.g. https://example.com/wp-json/blank-plugin/v1/reviews?q='Hello'&category=23,43&post_tag=23,32&page_no=1&posts_per_page=9
	 * e.g  https://example.com/wp-json/blank-plugin/v1/reviews?rating=4
	...
	private function register_user_meta(): void {
		$meta_fields = array(
			self::QUIZ_SCORE_KEY     => array(
				'type'        => 'integer',
				'description' => esc_html__( 'User quiz score', 'blank-plugin' ),
				'default'     => 0,
			),
			self::QUIZ_ANSWER_KEY    => array(
				'type'        => 'string',
				'description' => esc_html__( 'Answer for quiz question', 'blank-plugin' ),
			),
			self::QUIZ_CORRECT_KEY   => array(
				'type'        => 'string',
				'description' => esc_html__( 'Correctness of quiz answer (1 or 0)', 'blank-plugin' ),
			),
			self::QUIZ_TIMESTAMP_KEY => array(
				'type'        => 'string',
				'description' => esc_html__( 'Timestamp of quiz submission', 'blank-plugin' ),
			),
		);
	...
	private function get_route_args(): array {
		return array(
			'q'              => array(
				'required'          => false,
				'type'              => 'string',
				'description'       => esc_html__( 'Search query', 'blank-plugin' ),
			...
			'categories'     => array(
				'required'          => false,
				'type'              => 'string',
				'description'       => esc_html__( 'Comma-separated category IDs', 'blank-plugin' ),
				'sanitize_callback' => array( $this, 'sanitize_comma_separated_ids' ),
				'validate_callback' => array( $this, 'validate_comma_separated_ids' ),
			),
			'tags'           => array(
				'required'          => false,
				'type'              => 'string',
				'description'       => esc_html__( 'Comma-separated tag IDs', 'blank-plugin' ),
				'sanitize_callback' => array( $this, 'sanitize_comma_separated_ids' ),
				'validate_callback' => array( $this, 'validate_comma_separated_ids' ),
			),
			'page_no'        => array(
				'required'          => false,
				'type'              => 'integer',
				'description'       => esc_html__( 'Page number', 'blank-plugin' ),
			...
			'posts_per_page' => array(
				'required'          => false,
				'type'              => 'integer',
				'description'       => esc_html__( 'Posts per page', 'blank-plugin' ),
				'sanitize_callback' => 'absint',
				'validate_callback' => array( $this, 'validate_positive_integer' ),
			),
			'rating'         => array(
				'required'          => false,
				'type'              => 'string',
				'description'       => esc_html__( 'Rating value or range (e.g., 4.5 or 3.0-5.0)', 'blank-plugin' ),
