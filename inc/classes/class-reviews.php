...
class Reviews {
	...
	protected function setup_hooks() {
		add_shortcode( 'reviews', array( $this, 'render_shortcode' ) );
	}
	...
	public function render_shortcode( $atts ) {
		...
		// Get paged parameter safely.
		$paged = max( 1, get_query_var( 'paged' ) );
		if ( ! $paged && isset( $_GET['paged'] ) ) { // phpcs:ignore WordPress.Security.NonceVerification.Recommended
			$paged = absint( $_GET['paged'] ); // phpcs:ignore WordPress.Security.NonceVerification.Recommended
		}
		if ( ! $paged ) {
			$paged = 1;
		}
	...
	private function render_reviews( $reviews ) {
		...
		?>
<div class="blank-plugin-reviews" itemscope itemtype="http://schema.org/AggregateRating">
		<?php if ( ! $reviews->have_posts() ) : ?>
	<p><?php esc_html_e( 'No reviews found.', 'blank-plugin' ); ?></p>
	<?php else : ?>
		<?php
				$ratings = array();
		while ( $reviews->have_posts() ) :
			...
			?>
	<div class="blank-plugin-review" itemscope itemtype="http://schema.org/Review">
		<meta itemprop="author" content="<?php echo esc_attr( $review['reviewer_name'] ); ?>">
		<h3 itemprop="name"><?php the_title(); ?></h3>
			<?php if ( $review['product_id'] ) : ?>
		<p>
			<strong><?php esc_html_e( 'Product:', 'blank-plugin' ); ?></strong>
		...
		<p>
			<strong><?php esc_html_e( 'Reviewer:', 'blank-plugin' ); ?></strong>
