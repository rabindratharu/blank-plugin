...
if (0 === args.length) {
	rl.question('Would you like to setup the plugin? (Y/n) ', (answer) => {
		...
		rl.question('Enter plugin name (shown in WordPress admin)*: ', (pluginName) => {
			const pluginInfo = renderPluginDetails(pluginName);
...
/**
 * Renders the plugin setup modal with all necessary information related to the search-replace.
 *
 * @param {string} pluginName
 *
 * @return {Object} pluginInfo
 */
const renderPluginDetails = (pluginName) => {
	console.log(info.success('\nFiring up the plugin setup...'));

	// Bail out if plugin name isn't provided.
	if (!pluginName) {
...
const initPlugin = (pluginInfo) => {
	const chunksToReplace = {
		'blank plugin': pluginInfo.pluginNameLowerCase,
		'Blank Plugin': pluginInfo.pluginName,
		BlankPlugin: pluginInfo.pascalCase,
		'BLANK PLUGIN': pluginInfo.pluginNameCobolCase,
		'blank-plugin': pluginInfo.kebabCase,
		'Blank-Plugin': pluginInfo.trainCase,
		'BLANK-PLUGIN': pluginInfo.cobolCase,
		blank_plugin: pluginInfo.snakeCase,
		Blank_Plugin: pluginInfo.pascalSnakeCase,
		BLANK_PLUGIN: pluginInfo.macroCase,
		'blank-plugin-': pluginInfo.kebabCaseWithHyphenSuffix,
		'Blank-Plugin-': pluginInfo.trainCaseWithHyphenSuffix,
		'BLANK-PLUGIN-': pluginInfo.cobolCaseWithHyphenSuffix,
		blank_plugin_: pluginInfo.snakeCaseWithUnderscoreSuffix,
		Blank_Plugin_: pluginInfo.pascalSnakeCaseWithUnderscoreSuffix,
		BLANK_PLUGIN_: pluginInfo.macroCaseWithUnderscoreSuffix,
	};
