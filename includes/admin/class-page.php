...
class Page
{
    ...
    public function add_admin_menu(): void
    {
        ...
        add_menu_page(
            $this->menu_info['page_title'] ?? esc_html__('Blank Plugin Page', 'blank-plugin'),
            $this->menu_info['menu_title'] ?? esc_html__('Blank Plugin', 'blank-plugin'),
            'manage_options',
            $this->menu_info['menu_slug'] ?? BLANK_PLUGIN_NAME,
    ...
    public function is_menu_page(): bool
    {
        ...
        $admin_scripts_bases = ['toplevel_page_' . BLANK_PLUGIN_NAME];
    ...
    {
        printf('<div id="%s"></div>', esc_attr(BLANK_PLUGIN_NAME));
    }
    ...
    public function register_admin_assets(): void
    {
        ...
        // Register styles.
        wp_register_style('atomic', BLANK_PLUGIN_BUILD_PATH_URL . "/library/atomic-css/atomic.min{$suffix}.css", [], filemtime(BLANK_PLUGIN_BUILD_PATH . "/library/atomic-css/atomic.min{$suffix}.css"), 'all');
        wp_register_style('blank-plugin-admin', BLANK_PLUGIN_BUILD_PATH_URL . "/admin/index{$suffix}.css", ['atomic', 'wp-components'], filemtime(BLANK_PLUGIN_BUILD_PATH . "/admin/index{$suffix}.css"), 'all');

        // Enqueue Styles.
        wp_enqueue_style('blank-plugin-admin');


        $asset_config_file = sprintf('%s/admin/index.asset.php', BLANK_PLUGIN_BUILD_PATH);
        if (!file_exists($asset_config_file)) {
            if (defined('WP_DEBUG') && WP_DEBUG) {
                error_log('Blank_Plugin: Asset config file not found at ' . $asset_config_file);
            }
            return;
        }
        ...
        // Enqueue scripts.
        wp_enqueue_script('blank-plugin-admin');

        /* Localize */
        $localize = apply_filters(
            'blank_plugin_admin_localize',
            [
                'version'     => $version,
                'root_id'     => BLANK_PLUGIN_NAME,
