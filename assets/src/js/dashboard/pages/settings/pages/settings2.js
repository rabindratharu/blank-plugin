...
const MainContent = () => {
	...
	return (
		<AtrcContent>
			...
			<AtrcPanelRow>
				<AtrcControlSelect
					label={__('Setting 5 Select', 'blank-plugin')}
					value={setting5}
					options={[
						{
							label: __('Option 1', 'blank-plugin'),
							value: 'option-1',
						},
						{
							label: __('Option 2', 'blank-plugin'),
							value: 'option-2',
						},
					]}
					onChange={(newVal) => dbUpdateSetting('setting5', newVal)}
				/>
			</AtrcPanelRow>
		</AtrcContent>
	);
			...
			renderContent={
				<>
					<AtrcPanelBody
						...
						title={__('FAQ Query 3?', 'blank-plugin')}
						initialOpen={true}
					>
						<AtrcText
							tag="p"
						...
						>
							{__('FAQ Answer 3', 'blank-plugin')}
						</AtrcText>
					</AtrcPanelBody>
					<AtrcPanelBody
						title={__('FAQ Query 4?', 'blank-plugin')}
						initialOpen={false}
					>
						<AtrcText
							tag="p"
...
const Settings = () => {
	...
	return (
		<AtrcWireFrameHeaderContentFooter
			...
			renderHeader={
				<AtrcTitleTemplate1 title={__('Settings', 'blank-plugin')} />
			}
			renderContent={
				<AtrcWireFrameContentSidebar
					wrapProps={{
